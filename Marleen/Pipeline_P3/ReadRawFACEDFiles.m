function ReadRawFACEDFiles(DataFolder, SaveFolder, BinningSpatial, BinningTemp, b_SubROI, Overwrite_AcqInfoStream, Av_Frames)
...
% make AcqInfoStream
if ~exist('Overwrite_AcqInfoStream', 'var')
    Overwrite_AcqInfoStream = 0;
end

if ~exist([SaveFolder 'AcqInfos.mat'], 'file') || Overwrite_AcqInfoStream == 1
    ...
    load([SaveFolder 'AcqInfos.mat'], 'AcqInfoStream');
end

if ~exist('Av_Frames', 'var')
    % only used for calibration of x-axis
    Av_Frames = 0;
end
...
fprintf('Done. \n');
    ...
    function fColor = ChannelsSort(fList)
        ...
        for indF = 1:size(fList,1)
            ...
            if Av_Frames > 1 % MB added 
                % warning, if it's not a fitting nr, you might lose some
                % images.
                temp = uint16(zeros(size(Images,1), size(Images,2), floor(size(Images,3)/Av_Frames)));
                indtemp = 0;

                for ind = 1:Av_Frames:floor(size(Images,3)/Av_Frames)*Av_Frames
                    indtemp = indtemp+1;
                    temp(:,:,indtemp) = mean(Images(:,:,ind:ind+(Av_Frames-1)),3);
                end
                Images = temp;
                clear temp
            end
